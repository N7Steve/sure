class Budget < ApplicationRecord
  ...
  def transactions
    ...
    if user_id.present?
      account_ids = family.accounts.where(owner_id: user_id).data_visible.included_in_reports.select(:id)
      scope = scope.joins(:entry).where(entries: { account_id: account_ids })
    elsif current_user
      account_ids = family.accounts.accessible_by(current_user).data_visible.included_in_reports.select(:id)
      scope = scope.joins(:entry).where(entries: { account_id: account_ids })
    end
