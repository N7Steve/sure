class Entry < ApplicationRecord
  ...
  def from_scheduled_payment?
    scheduled_payment_entry.present? || transfer_scheduled_payment_entry.present?
  end

  def source_scheduled_payment
    (scheduled_payment_entry || transfer_scheduled_payment_entry)&.scheduled_payment
  end
  ...
  private
    ...
    def prevent_deletion_if_scheduled_payment_linked
      return if destroyed_by_association

      linked = ScheduledPaymentEntry.where(entry_id: id).or(
        ScheduledPaymentEntry.where(transfer_entry_id: id)
      ).exists?

      if linked
        errors.add(:base, "Cannot delete a transaction generated by a scheduled payment")
        throw :abort
      end
    end
end
