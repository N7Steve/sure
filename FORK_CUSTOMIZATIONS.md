...
- El enlace de históricos proyecta la cadencia hacia atrás aunque la fecha de inicio sea futura y siempre enlaza explícitamente el movimiento desde el que se creó la programación, sin someter esa elección manual a los filtros heurísticos. Para el resto exige la misma cuenta, moneda, nombre normalizado, categoría, comercio y conjunto de etiquetas, además del signo y tipo correctos; admite ±5 días y una variación de importe de ±5 % para importes fijos o ±40 % para importes variables estimados. También comprueba los dos extremos de transferencias y respeta omisiones/rechazos y huecos pendientes.
- El matcher bloquea cada `Entry` con `lock!` dentro de la transacción abierta por `ScheduledPayment#with_lock`. No usar `Entry#with_lock`: el lector `transaction` generado por `delegated_type` oculta el método transaccional de Rails e impide ejecutar el bloque. En `2432a3212` esto deja `source=not_provided` incluso con origen recibido y `candidate_results={}` aunque se recorran siete candidatos; no demuestra pérdida de parámetros, fallo de UUID ni un despliegue obsoleto. La regresión del controlador reproduce Suno en EUR, sin etiquetas, con el origen y siete candidatos adicionales e inicio futuro, y verifica enlaces, insignias e idempotencia. Validación de esta corrección limitada a análisis estático por petición del usuario; regresión pendiente de ejecución.
...
- **Previsiones** proyecta por separado el saldo de cada cuenta de efectivo a 1, 3, 6, 12 o 36 meses. Los movimientos futuros de Agenda son la fuente de verdad; el flujo variable restante se obtiene de hasta 12 meses completos, con ponderación por recencia y reducción robusta de valores atípicos. El mismo selector ofrece **Patrimonio V2**, que separa flujo residual (18 meses, decay 0,92), reserva irregular (12 meses) y rentabilidad porcentual de inversiones (36 meses, decay 0,97). La rentabilidad usa el índice temporal nativo ajustado por aportaciones cuando el proveedor lo ofrece (actualmente `return.index` de Indexa Capital); para el resto usa `balances.net_market_flows` y capital expuesto ponderado por fecha, y en inversiones sin posiciones históricas usa como fallback los ajustes de valoración netos de flujos. El flujo residual central se acumula linealmente, pero su dispersión usa `z * sigma * sqrt(t)`; la inversión se capitaliza y también amplía su incertidumbre con `sqrt(t)`. Aportaciones y retiradas no son rendimiento. Las transferencias entre activos incluidos son neutras y las que cruzan la frontera financiera conservan su signo económico. Agenda se aplica en su fecha, ajusta desde esa fecha el capital invertido y aprende la dispersión de importes estimados tras cuatro confirmaciones (fallback 15 %). `ScheduledPayment::ForecastBacktest` y `forecast:backtest` comparan V2 con la tendencia agregada anterior sin reconstruir Agenda histórica, que no está versionada; `CASHFLOW_Z` permite calibrar la cobertura y `forecast:diagnose_cashflow` expone el cálculo mensual del residual.
...
- **Incluida en mis finanzas**: forma parte del patrimonio, los informes y la actividad habitual.
- **Solo seguimiento (`exclude_from_reports`)**: permite consultar y sincronizar la cuenta, pero la excluye del patrimonio, los informes y las métricas. Las transferencias conservan su naturaleza de transferencia.
- **Fuera de mis finanzas (`cashflow_boundary`)**: implica `exclude_from_reports`; además, las transferencias que cruzan esta frontera se contabilizan como gasto al salir de mis finanzas y como ingreso al entrar. Es el tratamiento destinado a cuentas operativas compartidas como la cuenta de la hipoteca.
...
- Transferencias que cruzan la frontera de mis finanzas reclasificadas como salida/entrada; cambiar el tratamiento también reclasifica transferencias existentes.
- Las cuentas fuera de mis finanzas y archivadas pueden seguir sincronizándose; presentación, tratamiento financiero y ciclo de vida son conceptos separados.
...
- `InvestmentStatement` extendido para rendimiento de cartera, posiciones, flujos y métricas de roboadvisor. Las aportaciones netas descuentan retiradas y la rentabilidad del periodo procede de P&L de valoración, no de la mera presencia de transacciones.
- `Investment::RoboadvisorPerformance` centraliza el índice temporal ajustado por aportaciones de proveedores como Indexa, el P&L monetario y el fallback de valoraciones para carteras manuales.
- Las carteras gestionadas se detectan por subtipo (`roboadvisor`/`managed_fund`) o por proveedor Indexa, de modo que un plan Indexa conserva el subtipo fiscal `pension` sin desaparecer de las métricas de roboadvisor.
...
- En creación y edición, **importe y fecha comparten una única fila de dos columnas** para reducir la altura del formulario. En el detalle editable, naturaleza permanece en su propia fila. El formulario de nueva transferencia aplica la misma composición y muestra debajo un nombre opcional; si se deja vacío, conserva el nombre automático basado en las cuentas.
...
- Las navegaciones y refreshes Turbo no aplican fundido global al contenido principal: los morphs y las recargas de frames actualizan datos sin el parpadeo de opacidad anterior. Las páginas de cuenta recargan únicamente su frame tras sincronizar y Transacciones usa un frame familiar para altas y sincronizaciones, conservando el refresh completo como fallback en pantallas sin una región específica. Las barras laterales limitan su transición a anchura, opacidad y borde para evitar el antiguo efecto de rebote. Los grupos de cuentas (efectivo, inversiones, etc.) conservan por usuario y dispositivo su estado abierto/cerrado entre páginas y también durante refreshes por morph en la misma URL —incluido el toggle de vista compacta—, mientras siguen actualizando saldos y contenido; además animan suavemente altura, contenido y chevrón al desplegarse.
